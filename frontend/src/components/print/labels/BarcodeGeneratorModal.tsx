...
export function BarcodeGeneratorModal({
  ...
  const [activeTab, setActiveTab] = useState<'setup' | 'sheet_preview'>('setup');
  ...
  return (
    <>
      <Modal
        open={isOpen && !showFullscreenPreview}
        onClose={onClose}
        title="Print Barcode & Product Labels"
        size={activeTab === 'sheet_preview' ? 'xl' : 'lg'}
      >
        <div className="space-y-4 text-xs text-default">
          {/* Mode Switcher Header Tabs */}
          <div className="flex items-center justify-between border-b border-default pb-3">
            <div className="inline-flex rounded-xl bg-surface-sunken p-1 border border-default">
              <button
                type="button"
              ...
              >
                ...
                <span>Full Sheet Layout ({totalLabels} Stickers)</span>
              </button>
            </div>
          ...
          {activeTab === 'setup' ? (
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <div className="max-h-40 overflow-y-auto rounded-xl border border-default bg-surface divide-y divide-default">
                  {products.length === 0 ? (
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 bg-surface-sunken/40 p-3 rounded-xl border border-default text-[11px]">
                  {(
                    ...
                    <button
                      ...
                      className="flex items-center gap-2 text-left text-muted hover:text-default cursor-pointer"
                    >
                      {fields[key] ? (
            ...
            /* Tab 2: Embedded Sheet Preview */
            <div className="space-y-3">
              <div className="flex items-center justify-between p-2.5 bg-surface-sunken rounded-xl border border-default">
                <span className="text-xs font-semibold text-default">
                  A4 Sticker Sheet Layout Preview ({preset === 'small_35x25' ? '50 labels/sheet' : '28 labels/sheet'})
                </span>
                <button
                  type="button"
                  onClick={() => setShowFullscreenPreview(true)}
                  className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-surface border border-default text-[11px] font-semibold text-default hover:text-primary transition-colors cursor-pointer"
                >
                  <Eye className="size-3.5" />
              ...
              <div className="max-h-[50vh] overflow-auto rounded-xl border border-default bg-slate-900/40 p-4 flex justify-center">
                <div className="scale-75 origin-top print-doc shadow-xl">
                  <LabelSheet
                    items={products}
                    preset={preset}
                    format={format}
