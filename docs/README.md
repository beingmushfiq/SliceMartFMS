...
>
> **Last updated:** 2026-08-23 · **Phase:** 0 closed · Phase 1 in progress
...
| 2026-08-23 | §8 updated for the **tenancy-runtime delivery** (§7 item 29, committed `781e4da`): `app/Core` now exists — `TenantContext`, `BelongsToTenant`, `ResolveTenant` · `EnsureTenantActive` · `CorrelationId`, `ErrorResponse`, and the three route files wired through `bootstrap/app.php`. Header changed from "Phase 1 not started" to "Phase 1 in progress"; the Phase 1 row drops "no `app/Core` yet" and re-points the next action to **migration Wave 5 (master data A)**; the code-in-repository row is re-measured to **80 tests / 324 assertions** (table count stays twenty-four — Wave 5 is not started). Brings this file level with `DEVELOPMENT_STATUS.md`, which was a wave ahead. |
