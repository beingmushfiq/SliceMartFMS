...
| | |
|---|---|
| **Current phase** | Phase 1 — 🔄 **in progress.** Migration Waves 1 (platform), 2 (org), 3 (identity) and 4 (infrastructure) complete; Waves 5+, tenancy runtime, auth and RBAC outstanding. |
...
| **Next phase** | Phase 1 continues at **§7 item 29 — the tenancy runtime**: `app/Core`, `BelongsToTenant`, `ResolveTenant`, and the three route files. Migration Wave 5 (master data A) follows. |
| **Backend** | 🔄 Laravel 13.26.1 skeleton at `/backend`, PHP floor `^8.5`. Tooling real and passing: `phpstan.neon` (level 9, `checkModelProperties`) + `pint.json`. **Wave 1 platform (§4.6), Wave 2 org (§4.7), Wave 3 identity (§4.8) and Wave 4 infrastructure (§4.9) migrations written and verified.** Still absent: `app/Core`, `app/Modules`, tenancy runtime, API routes. |
...
| **Tests** | 🔄 Real, but foundation-only. Frontend **128 tests across 7 files**, all passing, covering the reliability layer (`ErrorBoundary`, `StateView`, `QueryBoundary`, `AsyncButton`, `logger`, `api/errors`, `api/queryClient`). Backend **77 tests / 316 assertions** — the PHP-floor guard plus the Wave 1–4 schema contracts. |
...
| **1** | Auth · Tenancy · RBAC · Design System | 🔄 Migration Waves 1 (§4.6), 2 (§4.7), 3 (§4.8) and 4 (§4.9) done. Waves 5+, tenancy runtime, auth, RBAC outstanding |
...
`BelongsToTenant` and `ResolveTenant`. Migrations are no longer absent — the
six Wave 1 platform tables (§4.6), four Wave 2 org tables (§4.7), seven Wave 3
identity tables (§4.8) and seven Wave 4 infrastructure tables (§4.9) exist and are
verified.

### 4.5 Test coverage

128 frontend tests over 7 files; 77 backend tests / 316 assertions. Frontend
...
and no MSW handler to test. The backend suites are schema contracts, not
behaviour — there is no model, Action or endpoint over those twenty-four tables
yet. Contract-level testing against `API_CONTRACT.md` begins with the first real
endpoint, which §7 item 29 gates behind the tenancy runtime.
...
| 29 | **Tenancy runtime** — `app/Core` (`TenantContext`, tenancy exceptions, the `Action` base contract), the `BelongsToTenant` trait (global scope + `creating` hook stamping the guarded `tenant_id` + logged `withoutTenantScope`), `ResolveTenant` + a correlation-id middleware, and `routes/api_platform.php` · `api_tenant.php` · `api_public.php` wired through `bootstrap/app.php`. No endpoint may precede them. **Contract confirmed this session, not yet built** — see the 2026-08-23 tenancy-runtime change-log entry for the full reading list and build order | `ARCHITECTURE.md` §2, §3.1–§3.4, §5.1, §5.7; `API_CONTRACT.md` §1.6, §2.3, §3.1–§3.2, §7, §9.1 |
| 30 | Phase 1 **Wave 5 — master data A**, per `DATABASE_DESIGN.md` §16. Note `units` is the deferred forward reference `production_lines.capacity_unit_id` waits on (§16.1 rule 2, closed in Wave 9) | `DATABASE_DESIGN.md` §16 |
...
**Wave 3 (identity) is done** (§4.8) and **Wave 4 (infrastructure) is done**
(§4.9). Start at §7 item 29 — the tenancy runtime (`app/Core`,
`BelongsToTenant`, `ResolveTenant` + a correlation-id middleware, the single
exception `render` mapping, the JWT `api` guard, and the three route files). Its
**full reading list, verified starting point, and seven-step build order are in
the 2026-08-23 tenancy-runtime change-log entry below** — the contract is
already read, so begin at build-order step 1 (`TenantContext` + tenancy
exceptions + the `Action` base). Do not rewrite the
Wave 1–4 migrations; extend them only through a new migration.
...
| 2026-08-23 | **Tenancy runtime (§7 item 29) — contract confirmed and build order fixed; no code written yet.** This session was specification-reading and starting-point verification only, so the ledger records a plan, not a delivery. **Starting point, verified by inspection not memory:** `backend/app/` contains only `Http/Controllers/Controller.php`, `Models/User.php`, `Providers/AppServiceProvider.php` — `app/Core`, `app/Modules`, `app/Support` **do not exist**; `backend/routes/` has only `console.php` and `web.php` — none of the three ARCHITECTURE §2 route files exist; `bootstrap/app.php`'s `withRouting()` has **no `api:` argument and no `then:` closure**, so even if the route files existed they would not load; `withMiddleware()` is an empty closure, so none of the §5.1 chain exists; `withExceptions()` already forces JSON for `api/*` but has **no `render` mapping**, so nothing yet emits the §2.3 envelope; `config/auth.php` defines only a **session** guard, so ADR-007's JWT `api` guard must be created before `Authenticate` can bind. `app/Models/User.php` still declares none of the 16 tenancy columns Wave 3 added, which `checkModelProperties` will flag the moment it is used in typed code — it needs `BelongsToTenant`, `SoftDeletes` and a full `@property` block. **Contract read end to end before writing any of it:** ARCHITECTURE §2 (folder layout + the three route-file names), §3.1 (the five enforcement layers and how each fails: request→401, query→silent invisibility, write→guarded `tenant_id` stamp, schema→composite key, test→fails closed), §3.2 (the resolution chain `Authenticate → claims → ResolveTenant loads tenant, asserts status → bind TenantContext`; `withoutTenantScope()` platform-only **and each use logged**; a queue job without tenant context **throws immediately, never runs unscoped**; suspended/past-due tenants are **read-only, not blank**), §3.3 (out-of-scope returns **403 naming the missing scope**, never 404 or an empty list), §3.4 (per-tenant connection works only because nothing bypasses the scope), §4.1/§4.3/§4.4 (JWT claims `sub`,`tenant_id`,`scopes`,`perm_version`,`jti`; RBAC `module.resource.action`; the never-trusted-from-client list), §5.1 (the middleware order to implement: `EnsureHttps → CorrelationId → Authenticate → ResolveTenant → EnsureTenantActive → Authorize → RateLimit → Idempotency → FormRequest → Controller → Action → Resource → AuditLog`), §5.7 (the exception→code map, implemented **once** in the single handler; `TenantSuspended`→**402** `TENANT_INACTIVE`; stack traces never returned in prod; every response carries the correlation id); and API_CONTRACT §1.6 (`X-Tenant` only for platform super-admin; a client-supplied body `tenant_id` is **ignored and logged as a security event** if it disagrees with the token), §2.3 (the error envelope shape), §3.1–§3.2 (the auth/authz codes to emit), §7 (the correlation-id pipeline: client → middleware adopts/generates → log context → queued jobs → outbound calls → response header **and** `error.correlation_id`), §9.1 (authenticated tenant source is **the JWT claim, nothing else**). Re-read the three shipped migrations that pin the runtime's contract: `tenants` (`status` ∈ `trial|active|past_due|suspended|cancelled`, past_due/suspended are read-only not a hard gate; `slug` globally unique because it resolves before any context exists), `users` (`tenant_id` **nullable, NULL = platform user**; `is_platform_user` a VIRTUAL generated `tenant_id is null`, so platform-ness is read from the DB not an app convention; both generated columns read-only), `user_scopes` (**NOT NULL `tenant_id`** so a platform user cannot hold a scope row; **no rows = whole tenant**, the table restricts and never grants — the inversion that makes §3.3 return 403-with-scope-name rather than an empty list). **Fixed build order for the next session:** (1) `app/Core/Tenancy/TenantContext.php` (request-scoped singleton holding tenant + company/branch scope set; downstream reads it, never a request field) + tenancy exceptions (`TenantSuspended`, `OutOfScope`, `TenantMismatch`) + the `final class` `Action` base contract (constructor-injected deps, explicit actor, `DB::transaction`, events inside the transaction handled after commit, per §5.2); (2) `app/Core/Tenancy/Concerns/BelongsToTenant.php` — global scope filtering by `TenantContext`, a `creating` hook stamping the **guarded** `tenant_id` (mass-assignment ignored), and a `withoutTenantScope` that logs every use; (3) `app/Core/Http/Middleware/CorrelationId.php` then `ResolveTenant.php` then `EnsureTenantActive.php`; (4) the single exception `render` mapping in `bootstrap/app.php` emitting the §2.3 envelope per the §5.7 map; (5) the JWT `api` guard in `config/auth.php`; (6) `routes/api_platform.php` · `api_tenant.php` · `api_public.php`, registered via `withRouting(api: …, then: …)` and the middleware chain via `withMiddleware()`; (7) tests — the **layer-5 cross-tenant isolation test** proving MODULE_MAP's Phase 1 exit criterion "a second tenant can be created and proven isolated", built on `SchemaTestCase::insertTenantWithPlan()` (called twice), plus a `withoutTenantScope`-logs-every-use test and a queue-job-without-context-throws test. Then the backend gate (`composer check`, counts **re-measured never carried forward** — the Wave 1 scanner will move again on any new migration) and a ledger + README truth-up. **Nothing above was built; the next session starts at build-order step 1.** |
